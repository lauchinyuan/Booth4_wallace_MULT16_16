...
$$
\begin{align}
sign &= \sum _{i=0}^{7} S_{i}\sum_{j=2i+15}^{31}2^j \\
&= \sum _{i=0}^{7}[(1-\overline{S_i})(2^{32}-2^{2i+15})] \\
&= \sum _{i=0}^{7}(\overline{S_i} \cdot2^{2i+15}) +10923 \times2^{17} \tag{18}
\end{align}
$$
(18)式中，`10923`为二进制`10101010101011`， 则 $1092 3 \times2^{17}$相当于在原先产生的部分积的第17、18、20、22、24、26、28、30位填充1，则符号位的值 $\sum _{i=1}^{8}(\overline{S_i} \cdot2^{2i+15})$的矩阵阵列如图29所示。
