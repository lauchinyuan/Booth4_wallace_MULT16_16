...
##### Wallace树结构
完成对符号位的编码后，对编码后的部分积进行部分积压缩。本设计采用Wallace Tree结构来进行部分积压缩，构建的Wallace树的结构如图28所示，使用两级4:2压缩过程，共计3个4:2压缩过程来得到最终的两个部分积`PPC2_1`、`PPC2_2`，每个“4：2压缩“过程由多个“4：2压缩器模块”和若干”3：2压缩模块“组成。
...
常用的3：2压缩电路如图31所示，这其实是两个3:2压缩器级联得到的，其进位输出`co`与进位输入`ci`无关。
...
对于图34中位置索引为6的位置上使用的“无进位4：2压缩模块”，不考虑进位时，即令4：2压缩模块的进位输入`ci`为0，此时4：2压缩电路可以化简成图35所示电路。分析该电路图可知，该电路的后半部分实现的其实时半加器的功能，故3：2压缩器+半加器即构成了“无进位输入的4：2压缩模块”。
...
对于图40中位置索引27上使用的“固定输入为0的4：2压缩模块”，令图32中的“标准4：2压缩模块”的`i2`为0，则4:2压缩模块变换为图44中的电路结构。结合前文分析，这个变换后的4：2压缩的前半部分是图30中的“等效异或门”少一个输出端的非门，即实际上变为“等效同或结构”。
...
“固定输入为0和1的4：2压缩模块”，令原来图23中的输入端`i0`为0，`i2`为1，则原来的4：2压缩器变为如图29的电路结构，与传统4：2压缩器比较可知，4：2压缩器的第一级3:2压缩器被一个非门(NOT)取代。
...
<center>图13. 前四个部分积矩阵图</center>

1. 位置索引为3:0的权位有效的数据输入不到两行，矩阵图空白的地方都是0，在这些权位上，天然就是两个部分积，故而无需额外处理即可作为压缩后部分积的输出。
2. 位置索引为5:4的索引，有效的部分积输入是3个，使用3:2压缩模块即可生成两个压缩后的部分积，在本设计中，如前文
