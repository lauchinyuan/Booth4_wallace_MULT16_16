...
<center>图45. 优化后的固定输入为1的4：2压缩模块</center>
...
为了更进一步确认本乘法器计算结果的正确性，在testbench里通过for循环遍历了输入被乘数`A_NUM`和乘数`B_NUM`的所有可能，并进行结果验证，仿真结果也表明，本设计的计算结果正确无误。仿真测试的部分结果如图52所示，从图中亦可观察到结算结果正确。

![](image/modelsim2.png)

<center>图52. 数据遍历验证部分结果</center>
