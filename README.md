...
$$
生成的部分积数据源`pp_source`实际上是17bit的，$pp\_source[17]$是 $pp\_source$的符号位扩展，故而$pp\_source[17]=pp\_source[16]$，因此(16)式可以改写为(17)式，直接使用一个非门即可实现数据输出。
$$
...
假设第 $i$各部分的符号位为 $S_i$ ，其中 $i$ 为1-8，对于图25中的符号位，将这些符号位扩展对应的数值相加，其值为(18)式:
$$
\begin{align}
sign &= \sum _{i=1}^{8} S_{i}\sum_{j=2i+15}^{31}2^j \\
&= \sum _{i=1}^{8}[(1-\overline{S_i})(2^{32}-2^{2i+15})] \\
&= \sum _{i=1}^{8}(\overline{S_i} \cdot2^{2i+15}) +10923 \times2^{17} \tag{18}
\end{align}
$$
(18)式中，`10923`为二进制`10101010101011`， 则 $10923 \times2^{17}$相当于在原先产生的部分积的第17、18、20、22、24、26、28、30位填充1，则符号位的值 $\sum _{i=1}^{8}(\overline{S_i} \cdot2^{2i+15})$的矩阵阵列如图29所示。
