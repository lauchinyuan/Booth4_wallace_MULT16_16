...
前文所述”符号位编码“过程中，其数据排列具有一定规律性，出现了很多固定输入值为0或者1的位置，本设计依据这一规律进行了优化，使得每个4：2压缩过程实际上是多种“4：2压缩器模块”和“3：2压缩器模块”的组合，其电路细节将在下一节进行详细讲解。
...
前文已经提及，本设计中”4：2压缩过程“用到了"4:2压缩器"和”3：2压缩器“，这两种压缩器的传统一种常用的电路结构已经在图3、图4中展示过。为了方便读者阅读，现再次展示这两种结构，常用的3：2压缩电路如图20所示。

![](image/full_adder.png)

<center>图20. 传统3:2压缩模块</center>

结合图20，3:2压缩模块的进位输出`co`的逻辑表达式可以化为(17)式：
$$
...
利用这一思路，优化后的3:2压缩器的电路结构如图21所示。

![](image/3_2_new.png)

<center>图21. 优化后的3:2压缩模块</center>

常用的3：2压缩电路如图22所示，这其实是两个3:2压缩器级联得到的，其进位输出`co`与进位输入`ci`无关。

![](image/4_2_circuit.png)

<center>图21. 传统4:2压缩模块</center>

以相同的连接方式，将经过优化后的3:2压缩模块级联，即可得到优化后的4:2压缩模块，其电路结构如图22所示。
